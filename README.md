...
Sealing is off until you point Chatwoot at a minidauth sidecar; unconfigured, it behaves exactly like upstream.
...
2. **Point Chatwoot at it.** Set these in its environment, then start Chatwoot as usual:
   ...
   Now contact names and phone numbers, and the body of every message are sealed before they reach the database.

3. **Grant a reader.** Sealed fields open only for a user the quorum granted the `crm-reader` role. Grant it to an agent by their user id, from the minidauth checkout:

   ```sh
   DEMO_UID=<chatwoot user id> ./bootstrap.sh
   ```

   Revoke it in minidauth and their reads go dark, with no change to Chatwoot.
<!-- minidauth-run:end -->
